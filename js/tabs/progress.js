  ...
  if (isPeriodView) {
    ...
    if (reviewTab === "weekly") {
      ...
      periodSelector.innerHTML = `
        ...
        <span class="text-[11px] font-bold px-2 text-center w-28 select-none">${MONTH_NAMES[reviewMonth]} ${reviewYear}</span>
...
function renderReviewMetrics() {
  ...
  getLinkedBlocks().forEach(b => {
    ...
    card.innerHTML = `
      ...
      <div class="value ${pct >= 100 ? 'text-green' : pct >= 50 ? 'text-amber' : 'text-red'}">${actual} / ${target}</div>
...
function renderReviewDayDetails(dateStr) {
  ...
  // MOOD_LABELS available from js/constants.js
  ...
  container.innerHTML = `
    ...
    <div class="space-y-3 pt-2">
      ...
      <div class="border-t border-border pt-2 space-y-1.5 text-xs">
        ${log.feelingScore ? `<div><span class="text-text-dim">Mood:</span> <strong class="text-text">${MOOD_LABELS[log.feelingScore] || log.feelingScore}</strong></div>` : ''}
